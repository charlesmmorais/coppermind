...
async def build_demo(session: ClientSession, output: Path = OUTPUT) -> dict:
    """Use the same semantic MCP scenario for the visual demo and live CI."""
    await call(
        session,
        "project_create",
    ...
    # Terminate both ends explicitly. A strict final ERC cannot accept the
    # original open chain, even once the rotated pin anchors are corrected.
    await call(
        session,
        "component_add",
        {
    ...
    await routed(session, "connect_incremental", {"net": "VIN", "pins": ["J1.1", "R1.1"]})
    await routed(session, "connect_incremental", {"net": "VOUT", "pins": ["R3.2", "J2.1"]})
    # Refresh the terminal resistor after its second neighbour is known.
    r3 = await routed(session, "component_orient_flow", {"reference": "R3", "force": True})

    # R1 was the original anchor. Once both semantic nets are known,
    # orient it using the same flow rule and reroute only VIN/N12.
    r1 = await routed(
        session,
