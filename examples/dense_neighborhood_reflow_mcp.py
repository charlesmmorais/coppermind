...
async def call(session: ClientSession, name: str, arguments: dict[str, Any] | None = None):
    ...
    print(f"\n>>> {name}")
    print(
        json.dumps(data, indent=2, ensure_ascii=False) if isinstance(data, (dict, list)) else data
    )
...
async def add_resistor(session: ClientSession, reference: str, value: str = "10k") -> None:
    await call(
        session, "component_add", {"reference": reference, "symbol": "Device:R", "value": value}
    )
...
async def build_demo(session: ClientSession, output: Path = OUTPUT) -> dict:
    await call(
        session,
        "project_create",
    ...
    print("Strategy:", reflow.get("strategy"))
    print("Neighborhood:", reflow.get("neighborhood"))
    print("Focus nets:", reflow.get("focus_nets"))
    ...
    print("Required focus gain mm:", reflow.get("required_focus_gain_mm"))
    print("Baseline total route mm:", reflow.get("baseline_route_length_mm"))
    print("Final total route mm:", reflow.get("route_length_mm"))
    print("Total route delta mm:", reflow.get("route_length_delta_mm"))
    print("Route length limit mm:", reflow.get("route_length_limit_mm"))
    print("Target from:", reflow.get("target_from"))
    print("Target to:", reflow.get("target_to"))
    print("Target candidates:", reflow.get("target_slot_candidates"))
    print("Routable candidates:", reflow.get("routable_candidates"))
